...
/**
** \brief _TBDAPP_ Table Initialization
**
** \par Description:
**      Initialize table
** 
...
** \return -1
*/
int32 _TBDAPP__InitTable()
{
    int32 iStatus = -1;

    /* Initialize _TBDTBL_ table */
    iStatus = _TBDAPP__Init_TbdTbl_Table();
    if (iStatus != CFE_SUCCESS)
    {
        CFE_EVS_SendEvent(_TBDAPP__TBL_ERR_EID, CFE_EVS_ERROR,
                          "_TBDAPP__InitTable(): _TBDAPP__Init_TbdTbl_Table() failed (0x%04X)\n",
                          (unsigned int)iStatus);
    }

    /*
    ** TODO: Add calls to other tables' initialization here.
    **       Their functions should be defined in _tbdApp__tblutils.h/c files.
    */
...
int32 _TBDAPP__InitApp()
{
    ...
    iStatus = _TBDAPP__InitTable();
    if (iStatus != CFE_SUCCESS)
    {
        CFE_EVS_SendEvent(_TBDAPP__INIT_ERR_EID, CFE_EVS_ERROR,
                          "_TBDAPP__InitApp(): _TBDAPP__InitTable() failed (0x%04X)\n",
                          (unsigned int)iStatus);
        CFE_EVS_SendEvent(_TBDAPP__INIT_ERR_EID, CFE_EVS_ERROR,
                          "_TBDAPP_: Failed to initialize application. Version %d.%d.%d.%d\n",
                          (int)_TBDAPP__MAJOR_VERSION, (int)_TBDAPP__MINOR_VERSION, (int)_TBDAPP__REVISION,
